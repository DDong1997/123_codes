...
subroutine restruct(f_hp,flx_hp,dt)
    ...
    real(kind=8) :: dt,g_p_r1(3),g_n_r1(3),g_p_r(3),g_n_r(3),alpha,q(3,3),iq(3,3)
...
subroutine roe(mid_u,mid_d,mid_a)
	...
	do i=1-mt,nx+mt-1
		mid_d(i)=(d(i)+d(i+1)+2*sqrt(d(i)*d(i+1)))*0.25_8
		mid_u(i)=(u(i)+sqrt(d(i+1)/d(i))*u(i+1))/(1+sqrt(d(i+1)/d(i)))
		mid_h(i)=(h(i)+sqrt(d(i+1)/d(i))*h(i+1))/(1+sqrt(d(i+1)/d(i)))
